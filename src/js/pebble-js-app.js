            ...
            if(req.status == 200) {
                ...
                if (responsebgs && responsebgs.length > 0) {
                   ...
                   // get NameofT1DPerson and IOB
                    ...
                    if ( (currentCalcRaw !== 0) && (opts.rawon == "1") ) {
                      values += ",1";  // Show calculated raw value  
                    } else {
                      values += ",0";  // Do not show calculated raw value                        
                    }
                    if ( (currentRawUnfilt !== 0) && (opts.urawon == "1") ) {
                      values += ",1";  // Show unfiltered raw value
                    } else {
                      values += ",0";  // Do not show unfiltered raw value                        
                    }
                    values += "," + opts.animateon; // Animation ON or OFF
                    values += "," + opts.mainfont; // Main Font Digital or Classic
                    values += "," + opts.batterydisplay; // Battery Display Emoji or Number
                    values += "," + opts.feeling; // Feeling field
